...
var onCorrectHtml = '<P class="noticeWrap"><SPAN class=txt-succ>$data$</SPAN></P>';
//var onErrorHtml = '<div class="newQipao">'+
//	'<div class="newQipaoArrow"></div>'+
//	'<div class="newQipaoCtt" >$data$</div>'+
//'</div>';
var onShowHtml = '';
...
(function($) {
	$.formValidator =
	{
		...
		setTipState : function(elem,showclass,showmsg)
		{
			...
			else
			{
				...
				else
				{
					...
					if(showclass!=""){
						if(showclass == 'onCorrect'){ //隐藏正确提示
							tip.html(html).removeClass('onError').addClass(showclass+" help-block").hide();
						}else if(showclass == 'onError'){
							var timeouthandl = $(tip).attr('timeouthandl')?$(tip).attr('timeouthandl'):'';
							if(timeouthandl){
							   window.clearTimeout(timeouthandl);
							   timeouthandl = window.setTimeout(hide_tip,3000);
							   $(tip).attr('timeouthandl',timeouthandl)
							}else{
							   timeouthandl = window.setTimeout(hide_tip,3000);
							   $(tip).attr('timeouthandl',timeouthandl)
							}
							function hide_tip(){
							   tip.removeClass('onError');
							}

							function wr(){
								tip.html(html).removeClass().addClass(showclass+" help-block").show();
							}
							from_err_handle_2 = window.setTimeout(wr,300)//配合日期控件
						}else{
							tip.html(html).removeClass().addClass(showclass+" help-block").show();
						}
				...
				var stype = elem.type;
				if(stype == "password" || stype == "text" || stype == "file"|| stype == "textarea"|| stype == "select-one" || stype == "select-multiple")
				{
					jqobj = $(elem);
					if(onShowClass!="" && showclass == "onShow"){
						jqobj.removeClass(onErrorClass).addClass(onShowClass)
					};
					if(onFocusClass!="" && showclass == "onFocus"){
						jqobj.removeClass(onErrorClass).addClass(onFocusClass)
					};
					if(onCorrectClass!="" && showclass == "onCorrect"){
						jqobj.removeClass(onErrorClass).addClass(onCorrectClass);
						$(jqobj).next('.select2').removeClass('newError');
					};
					if(onErrorClass!="" && showclass == "onError"){
							jqobj.removeClass(onErrorClass).addClass(onErrorClass);
							$(jqobj).next('.select2').addClass('newError');
					};
				}
			}
		},
		...
		showMessage : function(returnObj)
		{
			...
			intiConfig = $("body").data(elem.validatorGroup);
			if (!isValid)
			{
				showclass = "onError";
		...
		{
			if(  initConfig_setting.needValidator == false){
				return;
			}
			if(validatorGroup == undefined){validatorGroup = "1"};
					...
					if (name_list.indexOf("^"+name+"^") == -1) {
						...
						if (returnObj) {
							...
							if (!returnObj.isValid) {
								...
								errorlist[errorlist.length] = {errorMessage:errorMessage,errorId:returnObj.id}
